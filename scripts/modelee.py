...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        annotations1 = json.load(open('C:\Breathing_Bag_Defect\Mask_RCNN\Dataset_mix\\' + subset + '\\dataset.json'))
...
# %%
model_dir = ""
weight_path = "C:\\Breathing_Bag_Defect\\Mask_RCNN\\logs\\object20230508T0832\\mask_rcnn_object_0020.h5"
dataset_dir = "C:\\Breathing_Bag_Defect\\Mask_RCNN\\Dataset_mix"
image_directory = "C:\\Breathing_Bag_Defect\\Mask_RCNN\\Testset\\Test2"
...
for i in range(current_idx, len(image_paths)):
    ...
    print(current_idx)
    visualize.display_instances(current_image, results[0]['rois'], results[0]['masks'], results[0]['class_ids'], 
                                dataset.class_names, results[0]['scores'], ax=img_pr)

    img_pr.set_title(f"Predicted Image: {current_idx + 1}/{len(image_paths)}")
    img_pr.set_xticks([])
    img_pr.set_yticks([])

    current_idx = (current_idx + 1) % len(image_paths)

    plt.savefig(os.path.join("Mix_20_2", f"image_{current_idx}.png"))
#plt.show()
